...
class ChatHistoryDB:
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_file) as conn:
            ...
            """)
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_llm_provider ON chat_history(llm_provider)
            """)
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_tool_name ON chat_history(tool_name)
            """)
    ...
    def get_chat_history(self, limit: int = 100, filters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_file) as conn:
            ...
            if filters:
                ...
                if filters.get('parsing_mode'):
                    conditions.append("parsing_mode = ?")
                    params.append(filters['parsing_mode'])
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_file) as conn:
            ...
            stats['success_rate'] = (successful / stats['total_queries'] * 100) if stats['total_queries'] > 0 else 0
    ...
    def get_provider_breakdown(self) -> pd.DataFrame:
        """Get breakdown by LLM provider"""
        with sqlite3.connect(self.db_file) as conn:
            return pd.read_sql_query("""
                SELECT 
                    ...
                    AVG(elapsed_time_ms) as avg_response_time,
    ...
    def get_tool_breakdown(self) -> pd.DataFrame:
        """Get breakdown by tool usage"""
        with sqlite3.connect(self.db_file) as conn:
            return pd.read_sql_query("""
                SELECT 
                    tool_name,
...
# --- LLM Query Parser (Same as before but with model name tracking) ---
...
# --- Rule-based Parser (Same as before) ---
...
def history_analytics():
    ...
    with col1:
        provider_filter = st.selectbox("LLM Provider", ["All"] + LLM_MODELS)
    with col2:
        parsing_filter = st.selectbox("Parsing Mode", ["All", "LLM", "Rule-based"])
    with col3:
        success_filter = st.selectbox("Success Status", ["All", "Success", "Error"])
    with col4:
    ...
    if provider_filter != "All":
        filters['llm_provider'] = provider_filter
    if parsing_filter != "All":
        filters['parsing_mode'] = parsing_filter
    if success_filter != "All":
        filters['success'] = success_filter == "Success"
    if date_from:
    ...
    history = st.session_state.chat_history_db.get_chat_history(limit=limit, filters=filters)
    ...
    if history:
        ...
        # Display table
        st.subheader(f"📋 Query History ({len(df)} results)")
        ...
        display_columns = [
            'id', 'formatted_timestamp', 'session_id', 'llm_provider', 'model_name',
            'parsing_mode', 'user_query', 'tool_name', 'success', 'elapsed_time_ms'
        ]
        ...
        if len(df) > 0:
            ...
            with col1:
                ...
                st.json({
                    ...
                    "Timestamp": selected_entry['formatted_timestamp'],
                    "Session ID": selected_entry['session_id'],
                    "Query": selected_entry['user_query'],
                    "Provider": selected_entry['llm_provider'],
                    "Model": selected_entry['model_name'],
                    "Parsing Mode": selected_entry['parsing_mode']
                })
            
            with col2:
                st.markdown("**Execution Details:**")
                st.json({
                    "Action": selected_entry['parsed_action'],
                    "Tool": selected_entry['tool_name'],
                    "Resource URI": selected_entry['resource_uri'],
                    "Success": selected_entry['success'],
                    "Elapsed Time (ms)": selected_entry['elapsed_time_ms'],
                    "Confidence": selected_entry['confidence'],
                    "Error": selected_entry['error_message']
                })
            
            if selected_entry['reasoning']:
                st.markdown("**LLM Reasoning:**")
                st.info(selected_entry['reasoning'])
            
            if selected_entry['formatted_response']:
                st.markdown("**Response:**")
                st.markdown(selected_entry['formatted_response'])
    ...
    if stats['total_queries'] > 0:
        ...
        # Provider breakdown
        provider_df = st.session_state.chat_history_db.get_provider_breakdown()
        if not provider_df.empty:
            st.markdown("**Performance by LLM Provider:**")
            st.dataframe(provider_df)
        
        # Tool usage breakdown
        tool_df = st.session_state.chat_history_db.get_tool_breakdown()
...
def database_management():
    ...
    with col1:
        days_to_keep = st.number_input("Keep entries from last N days", min_value=1, max_value=365, value=30)
        
        if st.button("🗑️ Delete Old Entries", type="secondary"):
            try:
                cutoff_date = datetime.now() - timedelta(days=days_to_keep)
                
                with sqlite3.connect(DATABASE_FILE) as conn:
                    cursor = conn.cursor()
                    cursor.execute("SELECT COUNT(*) FROM chat_history WHERE timestamp < ?", (cutoff_date,))
                    count_to_delete = cursor.fetchone()[0]
                    
                    if count_to_delete > 0:
                        if st.confirm(f"This will delete {count_to_delete} entries older than {days_to_keep} days. Continue?"):
                            cursor.execute("DELETE FROM chat_history WHERE timestamp < ?", (cutoff_date,))
                            conn.commit()
                            st.success(f"✅ Deleted {count_to_delete} old entries.")
                    else:
    ...
    with col2:
        ...
        if st.button("🔥 Clear All Data", type="secondary"):
            if st.confirm("This will permanently delete ALL chat history. This cannot be undone!"):
                try:
                    with sqlite3.connect(DATABASE_FILE) as conn:
                        cursor = conn.cursor()
    ...
    with st.expander("Advanced: Run Custom SQL Query"):
        st.warning("⚠️ This is for advanced users. Only use SELECT queries for safety.")
        
        sql_query = st.text_area(
            "SQL Query:",
            value="SELECT * FROM chat_history ORDER BY timestamp DESC LIMIT 10;",
            height=100
        )
